...
#define DEGREES_MINUTES_SECONDES_TO_SECONDS(deg,min,sec) ((double)(deg)*3600+(double)(min)*60+(double)sec)
#define DEGREES_MINUTES_SECONDS_TO_DECIMAL_DEGREES(deg,min,sec) (deg) >= 0 ? ((double)(deg)+(double)(min)*0.0166666667+(double)(sec)*0.0002777778) : ((double)(deg)-(double)(min)*0.0166666667-(double)(sec)*0.0002777778)
...
#define HOURS_MINUTES_SECONDS_TO_SECONDS(hour,min,sec) ((double)(hour)*3600+(double)(min)*60+(double)sec)
#define HOURS_MINUTES_SECONDS_TO_DECIMAL_HOURS(hour,min,sec) ((double)(hour)+(double)(min)*0.0166666667+(double)(sec)*0.0002777778)
#define HOURS_TO_DEGREES(value) ((value)*15)

// Convert seconds
#define SECONDS_TO_DECIMAL_DEGREES(value) ((double)(value)*0.0002777778)
#define SECONDS_TO_DECIMAL_HOURS(value) ((double)(value)*0.0002777778)

#define T_WITH_JD(day,time)((day-2451545.0+time)/36525)

// Observer's coordinates on Earth
static double latitudeOnEarth      = NAN;
static double longitudeOnEarth     = NAN;
static double longitudeOnEarthSign = -1;
static int   altitudeOnEarth      = NAN;

void Ephemeris::floatingHoursToHoursMinutesSeconds(double floatingHours, int *hours, int *minutes, double *seconds)
{
...
double Ephemeris::hoursMinutesSecondsToFloatingHours(int degrees, int minutes, double seconds)
{
    return HOURS_MINUTES_SECONDS_TO_DECIMAL_HOURS(degrees, minutes, seconds);
}

void Ephemeris::floatingDegreesToDegreesMinutesSeconds(double floatintDegrees, int *degree, int *minute, double *second)
{
...
double Ephemeris::degreesMinutesSecondsToFloatingDegrees(int degrees, int minutes, double seconds)
{
                                                                                ...
                                                                                unsigned int hours, unsigned int minutes, unsigned int seconds)
{
    ...
    if( !isnan(longitudeOnEarth) && !isnan(latitudeOnEarth) )
    {
        ...
        double T = T_WITH_JD(jd.day,jd.time);
        
        double meanSideralTime = meanGreenwichSiderealTimeAtDateAndTime(day, month, year, hours, minutes, seconds);
        
        double deltaNutation;
        double epsilon = obliquityAndNutationForT(T, NULL, &deltaNutation);
        
        // Apparent sideral time in floating hours
        double theta0 = meanSideralTime + (deltaNutation/15*COSD(epsilon))/3600;
        
        
        // Geographic longitude in floating hours
        double L = DEGREES_TO_HOURS(longitudeOnEarth*longitudeOnEarthSign);
        
        // Geographic latitude in floating degrees
        double phi = latitudeOnEarth;
        
        // Local angle in floating degrees
        double H = (theta0-L-eqCoordinates.ra)*15;
...
double Ephemeris::apparentSideralTime(unsigned int day,  unsigned int month,  unsigned int year,
                                     unsigned int hours, unsigned int minutes, unsigned int seconds)
{
    JulianDay jd = Calendar::julianDayForDate(day, month, year);
    ...
    double theta0 = 100.46061837 + T*36000.770053608 + TSquared*0.000387933  - TCubed/38710000;
    theta0 = LIMIT_DEGREES_TO_360(theta0);
    theta0 = DEGREES_TO_HOURS(theta0);
    
    double time = HOURS_MINUTES_SECONDS_TO_DECIMAL_HOURS(hours,minutes,seconds);

	double r = 1.00273790935;
    
    double apparentSideralTime = theta0 + r * time;
    apparentSideralTime = LIMIT_HOURS_TO_24(apparentSideralTime);
    
    return apparentSideralTime;
}

double Ephemeris::obliquityAndNutationForT(double T, double *deltaObliquity, double *deltaNutation)
{
    double TSquared = T*T;
    double TCubed   = TSquared*T;
    
    double Ls = 280.4565 + T*36000.7698   + TSquared*0.000303;
    Ls = LIMIT_DEGREES_TO_360(Ls);
    
    double Lm = 218.3164 + T*481267.8812  - TSquared*0.001599;
    Lm = LIMIT_DEGREES_TO_360(Lm);
    
    double Ms = 357.5291 + T*35999.0503   - TSquared*0.000154;
    Ms = LIMIT_DEGREES_TO_360(Ms);
    
    double Mm = 134.9634 + T*477198.8675  + TSquared*0.008721;
    Mm = LIMIT_DEGREES_TO_360(Mm);
    
    double omega = 125.0443 - T*1934.1363 + TSquared*0.008721;
    ...
    double eps0 = DEGREES_MINUTES_SECONDES_TO_SECONDS(23,26,21.448)-T*46.8150-TSquared*0.00059+TCubed*0.001813;
    
    double obliquity = eps0 + dObliquity;
...
#if !DISABLE_PLANETS
double Ephemeris::sumELP2000Coefs(const double *moonMultCoefficients, const ELP2000Coefficient *moonAngleCoefficients, int coefCount,
                                 double E, double D, double M, double Mp, double F, bool squareMultiplicator)
{
    ...
    for(int numCoef=0; numCoef<coefCount; numCoef++)
    {
        ...
        memcpy_P(&multiplicator, &moonMultCoefficients[numCoef],  sizeof(double));
        ...
        double angle =  moonAngle.D*D + moonAngle.M*M + moonAngle.Mp*Mp + moonAngle.F*F;
...
#if !DISABLE_PLANETS
EquatorialCoordinates  Ephemeris::equatorialCoordinatesForEarthsMoonAtJD(JulianDay jd, double *distance)
{
    double T        = T_WITH_JD(jd.day,jd.time);
    double TSquared = T*T;
    double TCubed   = TSquared*T;
    ...
    double Lp = 218.31644735 + 481267.88122838*T - 0.001579944*TSquared + TCubed/538841 - T4/538841;
    Lp = LIMIT_DEGREES_TO_360(Lp);
    
    // Mean anomaly of the Sun
    double M = 357.5291092 + T*35999.0502909  - TSquared*0.0001536;
    M = LIMIT_DEGREES_TO_360(M);
    
    // Mean anomaly of Moon
    double Mp = 134.96339622 + 477198.86750067*T + 0.00872053*TSquared + TCubed/69699;
    Mp = LIMIT_DEGREES_TO_360(Mp);
    
    // Mean elongation of Moon
    double D = 297.85019172 + 445267.11139756*T - 0.00190272*TSquared + TCubed/545868;
    D = LIMIT_DEGREES_TO_360(D);
    
    // Mean distance at ascending node
    double F = 93.27209769 + 483202.01756053*T - 0.00367481*TSquared - TCubed/3525955;
    ...
    double E = 1 - 0.002516*T - 0.0000074*TCubed;
    E = LIMIT_DEGREES_TO_360(E);
    
    
    double sumL = sumELP2000Coefs( LMoonCoefficients, LMoonAngleCoefficients, sizeof(LMoonCoefficients)/sizeof(double), E, D, M, Mp, F, false);
    sumL += 3958*SIND(A1) + 1962*SIND(Lp-F) + 318*SIND(A2);
    
    double sumB = sumELP2000Coefs(BMoonCoefficients, BMoonAngleCoefficients, sizeof(BMoonCoefficients)/sizeof(double), E, D, M, Mp, F, false);
    sumB += -2235*SIND(Lp) + 382*SIND(A3) + 175*SIND(A1-F) + 175*SIND(A1+F) + 127*SIND(Lp-Mp) - 115*sin(Lp+Mp);
    
    double sumR = sumELP2000Coefs(RMoonCoefficients, RMoonAngleCoefficients, sizeof(RMoonCoefficients)/sizeof(double), E, D, M, Mp, F, true);
    
    // Geocentic longitude
    double lambda = Lp + sumL/1000000;   // Degrees
    
    // Geocentric latitude
    double beta   = sumB/1000000;        // Degrees
    
    // Distance
    double delta  = 385000.56+sumR/1000; // Kilometers
    
    // Convert kilometers to AU
    double dist = delta*6.68459e-9;
    ...
    double deltaNutation;
    double epsilon = Ephemeris::obliquityAndNutationForT(T, NULL, &deltaNutation);
    ...
    double paralax = 8.794/(dist)/3600.f;
    
    double meanSideralTime = meanGreenwichSiderealTimeAtJD(jd);
    ...
    // Local angle in floating degrees
    double H = (theta0-L-eqCoord.ra)*15;
    
    // Geocentric rectangular coordinates of the observer (Chap 6 p33)
    double u = ATAND(0.99664719*TAND(latitudeOnEarth));
    double rhoSinDeltaPrime    = 0.99664719*SIND(u)+(altitudeOnEarth/6378140)*SIND(latitudeOnEarth);
    double rhoCosDeltaPrime = COSD(u)+(altitudeOnEarth/6378140)*COSD(latitudeOnEarth);
    
    // Paralax correction (Chap 27 p 105)
    double detlaRa = ATAND(-rhoCosDeltaPrime*SIND(paralax)*SIND(H)/(COSD(eqCoord.dec)-rhoCosDeltaPrime*SIND(paralax)*COSD(H)));
    double detlaDec = ATAND((SIND(eqCoord.dec)-rhoSinDeltaPrime*SIND(paralax))*COSD(detlaRa)/(COSD(eqCoord.dec)-rhoCosDeltaPrime*SIND(paralax)*COSD(H)));
...
#if !DISABLE_PLANETS
EquatorialCoordinates  Ephemeris::equatorialCoordinatesForSunAtJD(JulianDay jd, double *distance)
{
    ...
    double L0 = 280.46646 + T*36000.76983 + TSquared*0.0003032;
    L0 = LIMIT_DEGREES_TO_360(L0);
    
    double M = 357.5291092 + T*35999.0502909  - TSquared*0.0001536;
    M = LIMIT_DEGREES_TO_360(M);
    
    double e = 0.016708634 - T*0.000042037 - TSquared*0.0000001267;
    ...
    {
        double Av = 351.52 + 22518.4428*T;  // Mars
        double Bv = 253.14 + 45036.8857*T;  // Venus
        double Cj = 157.23 + 32964.4673*T;  // Jupiter
        double Dm = 297.85 + 445267.1117*T; // Moon
        double E  = 252.08 + 20.19 *T;
        double H  = 42.43  + 65928.9358*T;
    ...
    // R
    double dist = (1.000001018*(1-e*e))/(1+e*COSD(v));
    if( distance ) *distance = dist;

    double omega = 125.04 - 1934.136*T;
    
    double lambda = O - 0.00569 - 0.00478 * SIND(omega);
    
    double deltaNutation;
    double epsilon = obliquityAndNutationForT(T, NULL, &deltaNutation);
    
    double eps = epsilon + 0.00256*COSD(omega);
...
#if !DISABLE_PLANETS
PlanetayOrbit Ephemeris::planetayOrbitForPlanetAndT(SolarSystemObjectIndex solarSystemObjectIndex, double T)
{
...
double Ephemeris::kepler(double M, double e)
{
    M = DEGREES_TO_RADIANS(M);
    
    double E = M;
    
    double previousE = E+1;
...
#if !DISABLE_PLANETS
double Ephemeris::sumVSOP87Coefs(const VSOP87Coefficient *valuePlanetCoefficients, int coefCount, double T)
{
    // Parse each value in coef table
    double value = 0;
    for(int numCoef=0; numCoef<coefCount; numCoef++)
    {
        ...
        double res = cos(coef.B + coef.C*T);
...
HorizontalCoordinates Ephemeris::equatorialToHorizontal(double H, double delta, double phi)
{
...
EquatorialCoordinates Ephemeris::horizontalToEquatorial(double azimuth, double altitude, double latitude)
{
...
EquatorialCoordinates Ephemeris::EclipticToEquatorial(double lambda, double beta, double epsilon)
{
...
double Ephemeris::meanGreenwichSiderealTimeAtDateAndTime(unsigned int day,   unsigned int month,   unsigned int year,
                                                        unsigned int hours, unsigned int minutes, unsigned int seconds)
{
    double meanGreenwichSiderealTime;
    
    JulianDay jd0 = Calendar::julianDayForDateAndTime(day, month, year, 0, 0, 0);
    double T0        = T_WITH_JD(jd0.day,jd0.time);
    double T0Squared = T0*T0;
    double T0Cubed   = T0Squared*T0;
    
    // Sideral time at midnight
    double theta0 = 100.46061837 + (36000.770053608*T0) + (0.000387933*T0Squared) - (T0Cubed/38710000);
    theta0 = LIMIT_DEGREES_TO_360(theta0);
    theta0 = DEGREES_TO_HOURS(theta0);
    
    // Sideral time of day
    double thetaH = 1.00273790935*HOURS_MINUTES_SECONDS_TO_SECONDS(hours,minutes,seconds);
...
double Ephemeris::meanGreenwichSiderealTimeAtJD(JulianDay jd)
{
    double meanGreenwichSiderealTime;
    
    double T0        = T_WITH_JD(jd.day,jd.time);
    double T0Squared = T0*T0;
    double T0Cubed   = T0Squared*T0;
                                                            ...
                                                            unsigned int hours, unsigned int minutes, unsigned int seconds)
{
    ...
    if( !isnan(longitudeOnEarth) && !isnan(latitudeOnEarth) )
    {
        ...
        // Local angle in floating degrees
        double H = (theta0-L-solarSystemObject.equaCoordinates.ra)*15;
        ...
        double T0 = Ephemeris::meanGreenwichSiderealTimeAtDateAndTime(day, month, year, 0, 0, 0);
        
        // Compute rise and set
        EquatorialCoordinates tmpCoord,tmpCoord0,tmpCoord24;
        double linearSpeedRA,linearSpeedDec;
        switch (solarSystemObjectIndex)
        {
            case Sun:
...
void Ephemeris::setLocationOnEarth(double floatingLatitude, double floatingLongitude)
{
    latitudeOnEarth  = floatingLatitude;
    longitudeOnEarth = floatingLongitude;
}

void Ephemeris::setLocationOnEarth(double latDegrees, double latMinutes, double latSeconds,
...
#if !DISABLE_PLANETS
EquatorialCoordinates  Ephemeris::equatorialCoordinatesForPlanetAtJD(SolarSystemObjectIndex solarSystemObjectIndex, JulianDay jd, double *distance)
{
    EquatorialCoordinates coordinates;
    coordinates.ra  = 0;
    coordinates.dec = 0;
    ...
    while (T != lastT)
    {
        ...
        double jd2 = (jd.day+jd.time) - TLight;
        T = T_WITH_JD(jd2,0);
    ...
    // Geocentic longitude
    double lambda = atan2(rectPlanet.y,rectPlanet.x);
    lambda = RADIANS_TO_DEGREES(lambda);
    lambda = LIMIT_DEGREES_TO_360(lambda);
    
    // Geocentric latitude
    double beta   = rectPlanet.z/sqrtf(x2+y2);
    ...
    // Remove abberation
    {
        ...
        double M = 357.52911 + T*35999.05029  - TSquared*0.0001537;
        M = LIMIT_DEGREES_TO_360(M);
        ...
        // Abberation
        double k = 20.49552;
        double xAberration = (-k*COSD(O - lambda) + earthOrbit.e*k*COSD(earthOrbit.pi - lambda)) / COSD(beta)/3600;
        double yAberration = -k*SIND(beta)*(SIND(O - lambda) - earthOrbit.e*SIND(earthOrbit.pi - lambda))/3600;
        lambda -= xAberration;
        beta   -= yAberration;
    }
...
#if !DISABLE_PLANETS
HeliocentricCoordinates  Ephemeris::heliocentricCoordinatesForPlanetAndT(SolarSystemObjectIndex solarSystemObjectIndex, double T)
{
...
RiseAndSetState  Ephemeris::riseAndSetForEquatorialCoordinatesAndT0(EquatorialCoordinates coord, double T0, double *rise, double *set,
                                                                    double paralax, double apparentDiameter)
{
    ...
    double lon = DEGREES_TO_HOURS(longitudeOnEarth*longitudeOnEarthSign);
    double lat = latitudeOnEarth;
    
    // Altitude angle
    double n1 = ACOSD(6378140/(double)(6378140 + altitudeOnEarth));
    ...
    double h0 = paralax - 34/60.0 -apparentDiameter/2 - n1;
    
    // cos H = (sin(h0) - sin(φ) sin(δ))/(cos(φ) cos (δ))
    double cosH = (SIND(h0) - SIND(lat)*SIND(coord.dec))/(COSD(lat)*COSD(coord.dec));
...
double Ephemeris::floatingHoursWithUTCOffset(float floatingHours, int UTCOffset)
{
                                                                            ...
                                                                            unsigned int year)
{
    ...
    double RADeg = HOURS_TO_DEGREES(eqEquinoxCoordinates.ra);
    
#if 1
    double Teq  = equinox-2000;
    double m    = 3.07496 + 0.00186*Teq; // S
    double nRA  = 1.33621 - 0.00057*Teq; // S
    double nDec = 20.0431 - 0.0085*Teq;  // "
    
    double deltaRA  = m + nRA * SIND(RADeg)*TAND(eqEquinoxCoordinates.dec);
    double deltaDec = nDec * COSD(RADeg);
#else
    ...
    double dec = eqJNowCoordinates.dec;
    
    double deltaPhi; // Delta nutation
    double deltaEps; // Delta obliquity
    double eps = obliquityAndNutationForT(T, &deltaEps, &deltaPhi);
    
    double deltaNutationRA = (COSD(eps) + SIND(eps) * SIND(RADeg) * TAND(dec))*deltaPhi - (COSD(RADeg)*TAND(dec)*deltaEps);
    deltaNutationRA = SECONDS_TO_DECIMAL_DEGREES(deltaNutationRA);
    deltaNutationRA = DEGREES_TO_HOURS(deltaNutationRA);
    
    double deltaNutationDec = (SIND(eps)*COSD(RADeg))*deltaPhi + SIND(RADeg)*deltaEps;
    ...
    double pi = 102.93735 + T*1.71946 + TSquared*0.00046;
